...
<script src="https://cdnjs.cloudflare.com/ajax/libs/handlebars.js/3.0.1/handlebars.js"></script>
  ...
  <script type="text/javascript" src="/resources/js/order/buyFromCart.js"></script>
		...
		<div class="col-xs-12">
			<table class="table table table-shopping-cart table-style">
				...
				<thead id="thead">
					<tr>
						<th>
						<input type="checkbox" id="checkAll" checked="checked" style="display:none"/></th>
						<th >이미지</th>
						<th >상품정보</th>
						<th >수량</th>
						<th >금액</th>
				...
				<c:forEach items="${productList}" var="productVO" varStatus="i">
					<tr id="productVO_${productVO.pdt_nb}" class="productRow">
						<td><input type="checkbox" name="check" class="check" value="${productVO.pdt_nb}" checked="checked" ></td>
						<td>
						...
						<td class="hidden-xs">
							${amountList[i.index]}
							<input type="hidden"  name="amount_${productVO.pdt_nb}" value="${amountList[i.index]}" />
							<input type="hidden"  name="cart_amount" value="${amountList[i.index]}" /> 
						</td> 
						<td class="hidden-xs">
						<p>
						<fmt:formatNumber value="${productVO.pdt_price}" pattern="###,###,###" /></p>
						<input type="hidden" name="price_${productVO.pdt_nb}" value="${productVO.pdt_price}" />
						</td>
						<td>
							...
							<input type="hidden"  class="form-control input-sm" id="memt_id" name="OrderDetailVOList[${i.index}].memt_id" value="${user.memt_id}">
							<input type="hidden" name="pdt_nb" value="${productVO.pdt_nb}" />
						</td>
						</tr>
					</c:forEach>	
			</table>
						...
						<table summary="" class="table table table-shopping-cart table-style">
							...
							<tbody>
								<tr>
										<td class="col-md-1">
										<p id="totalPrice">0</p>
										<input type="hidden" id="odr_totalby" name="odr_totalby" value="0"/>
										</td>
								</tr>
							</tbody>
						</table>
